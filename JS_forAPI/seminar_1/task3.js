...
// articlesEl.innerHTML = articles.map(article => { //фигурные значит return
//     return `
//         <div class="article">
...
articlesEl.addEventListener("click", ({ target }) => {
  if (target.closest(".removeBtn")) {
    ...
    const articles = getLsArticles();
    const foundedIndex = articles.findIndex(
  ...
  if (target.closest(".editBtn")) {
    ...
    if (!editingArticle) {
      ...
      return; //чтоб не делать допложенность в виде else
    }
...
document.querySelector(".addBtn").addEventListener("click", () => {
  ...
  const articles = getLsArticles();
  const newArticle = { id, title, content }; //ctrl+delete - удалять много пробелов
...
//2:23:00+ - не поняла про гет артикл хтмл
